...
#define NPY_ARRAY_MAGIC_STRING { (char)0x93, 'N', 'U', 'M', 'P', 'Y' }
...
size_t npy_array_get_header(const npy_array_t *m, char *buf)
{
    ...
    for (int i = 0; i < m->ndim; i++) {
        size_t remaining = NPY_ARRAY_SHAPE_BUFSIZE - (ptr - shape);
        int written = snprintf(ptr, remaining, "%d,", (int)m->shape[i]);

        if (written > 0 && (size_t)written < remaining) {
            ptr += written;
        }
        else if (written > 0) {
            ptr += remaining - 1; // Safely handle truncation
        }
    }
...
#define HEADER_LEN 108
    ...
    int len_written = snprintf(dict, NPY_ARRAY_DICT_BUFSIZE, "{'descr': '%c%c%zu', 'fortran_order': %s, 'shape': (%s), }", _guess_endianness(m), m->typechar, m->elem_size, m->fortran_order ? "True" : "False", shape);
    size_t len = (len_written > 0) ? (size_t)len_written : 0;

    assert(len < HEADER_LEN); /* FIXME: This can go wrong for really big arrays with a lot of dimensions */

    // Safely append the padding to the dictionary
    if (len < NPY_ARRAY_DICT_BUFSIZE) {
        int pad_written = snprintf(dict + len, NPY_ARRAY_DICT_BUFSIZE - len, "%*s\n", (int)(HEADER_LEN - len + NPY_ARRAY_PREHEADER_LENGTH - 1), " ");
        if (pad_written > 0 && (size_t)pad_written < (NPY_ARRAY_DICT_BUFSIZE - len)) {
            len += pad_written;
        }
        else if (pad_written > 0) {
            len = NPY_ARRAY_DICT_BUFSIZE - 1; // Cap at max buffer length if truncated
        }
    }
...
static const char *find_header_item(const char *item, const char *header)
{
    const char *s = strstr(header, item);
...
npy_array_t *_read_matrix(void *fp, reader_func read_func)
{
    ...
    npy_array_t *m = (npy_array_t *)calloc(1, sizeof *m);
    if (!m) {
        fprintf(stderr, "Cannot allocate memory dor matrix structure.\n");
        return NULL;
    }

    const char *descr = find_header_item("'descr': '", header);
    assert(descr);
    if (strchr("<>|", descr[0])) {
    ...
    const char *fortran = find_header_item("'fortran_order': ", header);
    assert(fortran);
    ...
    /* FIXME: This only works if there is one and only one leading spaces. */
    char *shape = (char *)find_header_item("'shape': ", header);
    assert(shape);
    while (*shape != ')') {
    ...
    m->data = (char *)malloc(n_elements * m->elem_size);
    if (!m->data) {
...
npy_array_t *npy_array_mmap(const char *filename)
{
    ...
    char *data = (char *)mmap(0, len, PROT_READ, MAP_SHARED, fd, 0);
    close(fd);
    if (data == MAP_FAILED) {
        perror("mmap failed!");
        return NULL;
    ...
    map_handler_t mh = { .start_pos = (char *)data, .current_pos = (char *)data, .length = (size_t)len };
...
void npy_array_free(npy_array_t *m)
{
    ...
    if (m->map_addr) {
        ...
        uint16_t header_length = 0;
        char *preheader = (char *)m->map_addr;
