...
/*
 * Modified to not use std::ranges
 */
...
public:
    ...
    [[nodiscard]] constexpr static char tolower(const char chr) noexcept
    {
...
{
public:
    te_expr(const te_variable_flags type, const te_variant_type &value) noexcept : m_type(type), m_value(value)
    {
    }
...
public:
    ...
    te_parser(const te_parser &that) : m_customFuncsAndVars(that.m_customFuncsAndVars), m_unknownSymbolResolve(that.m_unknownSymbolResolve), m_keepResolvedVariables(that.m_keepResolvedVariables), m_decimalSeparator(that.m_decimalSeparator), m_listSeparator(that.m_listSeparator), m_expression(that.m_expression)
    {
        try {
            if (!m_expression.empty()) {
                [[maybe_unused]] const auto retVal = evaluate(m_expression);
            }
        }
    ...
    /** @brief Compiles and evaluates an expression and returns its result.
        ...
        @throws std::runtime_error Throws an exception in the case of arithmetic overflows
            (e.g., `1 << 64` would cause an overflow).*/
    [[nodiscard]] te_type evaluate(std::string_view expression);

    /// @returns The last call to evaluate()'s result (which will be NaN on error).
    [[nodiscard]] te_type get_result() const noexcept
    {
        return m_result;
    }
    ...
    {
        const auto foundVar = m_customFuncsAndVars.find(te_variable{ std::move(var), static_cast<te_type>(0.0), TE_DEFAULT, nullptr });
    ...
    void remove_unused_variables_and_functions()
    {
        ...
        /* in loop*/) {
            funcIter = (is_function_used(funcIter->m_name) || is_variable_used(funcIter->m_name)) ? ++funcIter : m_customFuncsAndVars.erase(funcIter);
        }
    }
#endif
    ...
    [[nodiscard]] std::set<te_variable> &get_variables_and_functions() noexcept
    {
        return m_customFuncsAndVars;
    }
    ...
    ///     (not NaN or infinite).
    /// @private
    [[nodiscard]] static bool number_to_bool(te_type val)
    {
...
private:
    ...
    static void validate_name(const te_variable &var)
    {
        ...
        if (!is_letter(var.m_name[0]) && var.m_name[0] != '_') {
            throw std::runtime_error(std::string("Variable name must begin with a letter from a-z or _: ") + var.m_name);
        }
        const auto varCharPos = std::find_if(var.m_name.cbegin(), var.m_name.cend(), [](const auto chr) noexcept { return !is_name_char_valid(chr); });
    ...
    [[nodiscard]] constexpr static bool is_name_char_valid(const char chr) noexcept
    {
    ...
    [[nodiscard]] std::set<te_variable>::iterator find_variable_or_function(const std::string_view name)
    {
        ...
        return m_customFuncsAndVars.find(te_variable{ te_variable::name_type{ name }, static_cast<te_type>(0.0), TE_DEFAULT, nullptr });
    }

    /// @returns An iterator to the custom variable or function with the given @c name,
    ///     or end of get_variables_and_functions() if not found.
    /// @param name The name of the function or variable to search for.
    [[nodiscard]] std::set<te_variable>::const_iterator find_variable_or_function(const std::string_view name) const
    {
        if (name.empty()) {
    ...
    [[nodiscard]] static auto get_arity(const te_variant_type &var)
    {
        return std::visit(
    ...
    [[nodiscard]] constexpr static bool is_function(const te_variant_type &var)
    {
        return std::visit(
...
#undef TE_DEF_FUNCTION
    ...
    [[nodiscard]] constexpr static bool is_closure(const te_variant_type &var)
    {
        return std::visit(
        [](const auto &var0) -> bool {
            using T = std::decay_t<decltype(var0)>;
...
#undef TE_DEF_CLOSURE
    ...
    struct state
    {
        ...
        state(const char *expression, te_variable_flags varType, std::set<te_variable> &vars) : m_start(expression), m_next(expression), m_varType(varType), m_lookup(vars)
        {
        }
    ...
    [[nodiscard]] static te_expr *new_expr(const te_variable_flags type, te_variant_type value, const std::initializer_list<te_expr *> &parameters)
    {
        auto *ret = new te_expr{ type, value };
        ret->m_parameters.resize(std::max<size_t>(std::max<size_t>(parameters.size(), get_arity(ret->m_value)) + (is_closure(ret->m_value) ? 1 : 0), 0));
    ...
    [[nodiscard]] static te_expr *new_expr(const te_variable_flags type, te_variant_type value)
    {
        auto *ret = new te_expr{ type, value };
        ret->m_parameters.resize(static_cast<size_t>(get_arity(ret->m_value)) + (is_closure(ret->m_value) ? 1 : 0));
        return ret;
    }

    [[nodiscard]] constexpr static bool is_letter(const char chr) noexcept
    {
    ...
    [[nodiscard]] te_expr *te_compile(std::string_view expression, std::set<te_variable> &variables);
    ...
    {
        return m_functions.find(te_variable{ te_variable::name_type{ name }, static_cast<te_type>(0.0), TE_DEFAULT, nullptr });
    }

    [[nodiscard]] static auto find_lookup(state *ste, const std::string_view name)
    {
        return ste->m_lookup.find(te_variable{ te_variable::name_type{ name }, static_cast<te_type>(0.0), TE_DEFAULT, nullptr });
    }
